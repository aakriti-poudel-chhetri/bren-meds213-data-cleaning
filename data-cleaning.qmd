...
Decimal points don't really make sense for a percent cover, so we will round those off to whole integers.
```{r}
unique(snowsurvey_fixed$Snow_cover)
```

```{r}
snowsurvey_fixed$Snow_cover <- round(snowsurvey_fixed$Snow_cover)
unique(snowsurvey_fixed$Snow_cover)
```
